...
def create_image_patches(image, grid_size=24):
    """
    Convert image to PIL format and split into equal sized patches
    
    Args:
        image_array:List: Input image as numpy array
        grid_size:int: Number of rows/columns to split image into (default 24x24 grid)
        
    Returns:
        list: List of PIL Image patches
    """

    img = Image.fromarray(image[...,::-1]).resize((336, 336)) 
